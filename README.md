...
- **牌组管理**：新建 / 删除牌组，新建时可指定模板（也可随时修改牌组使用的模板），查看每个牌组的卡片数与待复习数量；支持选择 CSV 文件**导入**（微信端从会话选择，Android/iOS 端调用系统文件选择器）；支持**导出/导入牌组文件（JSON）**，可选是否带上学习进度，且始终带上模板
...
除 Anki CSV 外，本项目支持导出/导入**自有的牌组文件（JSON）**，用于备份、换机迁移或分享牌组。在「牌组」页：

- 每个牌组的操作区有「**导出**」：可选择
  - **导出牌组 + 学习进度**：连同每张卡片的 FSRS 记忆状态一起导出，导入后可无缝接续复习。
  - **仅导出牌组（不含进度）**：只导出卡片内容，导入后所有卡片作为新卡从零开始学。
- 无论哪种方式，**都会自动带上牌组绑定的模板**（字段定义、正反面模板、放大倍数），导入后可直接按模板继续加卡、重新渲染。
- 页头有「**导入牌组**」：选择 `.json` 文件即可导入。文件选择按环境自动切换（微信端从会话选择、Android/iOS 端系统文件选择器），与 CSV 导入一致。

### 数据格式与兼容性

导出文件为版本化 JSON（`utils/exchange.js`）：

```jsonc
{
  "format": "anki-mini-app-deck",   // 固定标识，用于识别文件
  "version": 1,                      // 整数版本号，用于未来迁移
  "exportedAt": 1700000000000,       // 导出时间戳
  "includeProgress": true,           // 是否带学习进度
  "template": {                      // 牌组绑定的模板（无模板牌组为 null）
    "name": "单词卡",
    "fields": ["单词", "释义"],
    "front": "{{单词}}",
    "back": "{{释义}}",
    ...
    "cards": [
      {
        "front": "apple",
        "back": "苹果",
        "fields": { "单词": "apple", "释义": "苹果" },  // 模板卡片保存原始字段值
        "srs": { "stability": 3.1, "difficulty": 5, "state": "review", ... }  // 仅 includeProgress 时存在
      }
    ]
  }
}
...
- **不存运行时细节**：卡片/牌组的内部自增 `id`、`createdAt` 等不导出，导入时一律重新生成，避免跨设备 id 冲突。
- **宽松解析、向前兼容**：导入只读取已知字段、忽略未知字段，因此旧版 App 打开新版导出文件不会因多出字段而报错；新版读旧文件时缺字段走默认值。
- **版本护栏**：`version` 高于当前 App 支持的最大版本时明确提示「文件由更新版本导出」，而非静默丢数据；`version` 只在结构无法安全兼容时才递增，增量字段不升版本。
- **模板复用**：导入时若已存在名称、正反面、字段集合完全一致的模板，直接复用而不重复创建；否则新建模板。
- **进度接续**：带进度导入的 `srs` 会经 `srs.normalize` 规整（兼容旧版 SM-2 结构），无进度则作为新卡立即可学。
...
│   ├── exchange.js     # 牌组导入/导出数据格式（版本化 JSON，带模板与可选进度）
